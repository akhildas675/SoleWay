...
const categoryAdding = async (req, res) => {
    try {
        ...
        const categoryExisting = await Category.findOne({ categoryName: { $regex: new RegExp('^' + categoryName + '$', 'i') } });
...
const categoryBlocking=async (req,res)=>{
    try {
        ...
        const categoryBlock=await Category.findByIdAndUpdate(categoryId,{is_active:false})
...
const categoryUnblocking=async (req,res)=>{
    try {
        ...
        const categoryUnBlock=await Category.findByIdAndUpdate(categoryId,{is_active:true})
...
const editingCategory= async (req,res)=>{
    try {
        ...
        const categoryExisting = await Category.findOne({_id: { $ne: req.session.categoryId}, categoryName: { $regex: new RegExp('^' + categoryName + '$', 'i') } });

        if (categoryExisting) {
            console.log(categoryData);
            res.redirect('/admin/categoryAdd?error=exists');
        } else {
            const updated = await Category.findByIdAndUpdate({ _id:  req.session.categoryId}, { $set: {  categoryName, description } })
